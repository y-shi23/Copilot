...
const normalizedEditingServerType = computed({
  get() {
    ...
    if (
      editingServer.type &&
      (streamableHttpRegex.test(editingServer.type) || editingServer.type.toLowerCase() === 'http')
    ) {
      return 'http';
    }
...
const filteredMcpServersList = computed(() => {
  ...
  return mcpServersList.value.filter(
    (server) =>
...
function getDisplayTypeName(type) {
  ...
  switch (lowerType) {
    case 'sse':
      return t('mcp.typeOptions.sse');
    case 'stdio':
      return t('mcp.typeOptions.stdio');
    default:
      return type;
  }
}
...
async function saveJson() {
  try {
    ...
    if (
      ...
      parsedJson === null ||
      !parsedJson.hasOwnProperty('mcpServers')
    ) {
...
async function runToolTest() {
  ...
  try {
    ...
    const configToTest = {
      ...
      headers:
        typeof server.headers === 'string' ? convertTextToObject(server.headers) : server.headers,
      args: Array.isArray(server.args) ? server.args : convertTextToLines(server.args),
    };

    const response = await window.api.testInvokeMcpTool(
      configToTest,
    ...
    if (response.success) {
      ...
      if (typeof rawResult === 'string') {
        ...
        testOutput.value = rawResult.content
          .map((item) => {
...
async function handleTestClick(server) {
  ...
  try {
    ...
    if (
      cache &&
      cache[server.id] &&
      Array.isArray(cache[server.id]) &&
      cache[server.id].length > 0
    ) {
  ...
  <div class="page-container">
                ...
                <div class="mcp-header-actions">
                  ...
                  >
                    <el-button
                      ...
                      @click.stop="
                        handleSwitchChange(server.id, 'isPersistent', !server.isPersistent)
                      "
                ...
                <div class="mcp-actions">
                  <el-tooltip :content="t('mcp.testConnectionTooltip')" placement="top">
                    <el-button
                      ...
                      text
                      circle
                      @click="handleTestClick(server)"
                      class="action-btn-compact"
                    />
                  </el-tooltip>
                  <el-button
                    v-if="server.type !== 'builtin'"
                    :icon="Edit"
                    text
                    circle
                    @click="prepareEditServer(server)"
                    class="action-btn-compact"
                  />
                  <el-button
                    v-if="server.type !== 'builtin'"
    ...
    >
      <el-form :model="editingServer" label-position="top" @submit.prevent="saveServer">
        <el-row :gutter="20">
          ...
          <el-col :span="6" class="switches-container">
            ...
            <el-form-item class="compact-form-item">
              ...
              <el-switch
                v-model="editingServer.isPersistent"
        ...
        <template
          v-if="normalizedEditingServerType === 'http' || normalizedEditingServerType === 'sse'"
        >
          <el-form-item :label="t('mcp.http.url')"
            ><el-input v-model="editingServer.baseUrl"
          /></el-form-item>
        ...
        <el-collapse v-model="advancedCollapse" class="advanced-collapse">
          <el-collapse-item :title="t('mcp.advanced')" name="1">
            ...
            <el-form-item :label="t('mcp.providerUrl')"
              ><el-input v-model="editingServer.providerUrl"
            /></el-form-item>
            <el-form-item :label="t('mcp.tags')"
              ><el-input v-model="editingServer.tags" :placeholder="t('mcp.tagsPlaceholder')"
            /></el-form-item>
          </el-collapse-item>
        </el-collapse>
      </el-form>
    ...
    >
        ...
        <div class="test-dialog-header" style="align-items: center">
          <div class="header-left">
            <span class="dialog-title">
              <el-tooltip
                v-if="testResult.serverDescription"
                :content="testResult.serverDescription"
      ...
      <div v-else class="test-result-content">
        <el-alert
          v-if="testResult.success && !testResult.isCached"
          :title="t('mcp.connectionSuccess', { count: testResult.tools.length })"
          type="success"
          show-icon
                ...
                <div class="tab-pane-content">
                  ...
                  <div class="tools-compact-list">
                    ...
                    >
                      ...
                      <el-tooltip
                        :content="tool.description || t('mcp.noDescription')"
                        placement="top"
                        :show-after="500"
                        popper-class="mcp-tooltip-width"
                      >
                        <span class="tool-compact-desc">{{
                ...
                <div class="tab-pane-content">
                  <div class="test-form-area no-border">
                    ...
                    <div class="test-form-row">
                      ...
                      <el-select
                        v-model="currentTestToolName"
                        :placeholder="t('mcp.test.selectToolPlaceholder')"
                        filterable
                        @change="handleTestToolChange"
                        style="flex: 1"
                      >
                        <el-option
                          v-for="tool in testResult.tools"
                    ...
                    <div v-if="currentTestToolName" class="dynamic-form-container">
                      ...
                      <div v-else class="form-grid">
                        <div
                          v-for="field in currentTestToolSchema"
                          :key="field.key"
                          class="dynamic-field"
                        >
                          ...
                          <el-input
                            ...
                            :placeholder="
                              field.default !== undefined
                                ? `${t('mcp.test.default')}: ${field.default}`
                                : t('mcp.test.leaveEmpty')
                            "
                            style="width: 100%"
                          />
        ...
        <el-empty
          v-else-if="testResult.success"
          :description="t('mcp.noToolsProvided')"
          :image-size="60"
        >
          <template #image>
            <el-icon :size="50" color="#909399">
...
.mcp-edit-dialog .el-dialog__body {
  height: 55vh;
  overflow-y: auto;
  overflow-x: hidden;
}

.mcp-edit-dialog .el-dialog__body::-webkit-scrollbar {
  width: 0;
  display: none;
}
</style>
