...
const isMacOS = computed(() => {
  const platform = String(launcherApi.platform || navigator.userAgentData?.platform || navigator.platform || '').toLowerCase()
...
watch(filteredPrompts, async (list) => {
  if (list.length === 0) {
...
function getPromptInitial(prompt) {
  return String(prompt?.code || '?').slice(0, 2).toUpperCase()
}
...
function executeCurrent(useTextPayload) {
  ...
  if (!attachment.value && shouldUseTextPayload && currentQuery && !supportsTextPayload(prompt, currentQuery)) {
...
async function handlePaste(event) {
  ...
  const imageData = typeof launcherApi.readClipboardImage === 'function'
    ? launcherApi.readClipboardImage()
    : ''
...
<template>
  <main class="launcher-shell" :class="{ dragging: isDragging, 'native-vibrancy': isMacOS, 'fallback-vibrancy': !isMacOS }">
    <div class="launcher-container" :class="{ 'has-list': isListVisible && filteredPrompts.length > 0 }">
      ...
      <section v-show="isListMounted" ref="listRef" class="result-list" :class="{ visible: isListVisible }">
        <div v-if="filteredPrompts.length === 0" class="result-empty">
          无匹配助手
        </div>
...
.launcher-container {
  ...
  box-shadow: var(--shadow-md), 0 0 0 1px color-mix(in srgb, #ffffff 18%, transparent);
...
.launcher-shell:focus-within .launcher-container {
  ...
  box-shadow: var(--shadow-md), 0 0 0 1px color-mix(in srgb, var(--text-accent) 16%, transparent);
}
