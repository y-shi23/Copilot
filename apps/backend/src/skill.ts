...
function loadBuiltinModule() {
  try {
    return require('./mcp_builtin');
  } catch (_error) {
    // Vitest executes TS sources directly; fall back to explicit TS extension.
    return require('./mcp_builtin.ts');
  }
}
...
function getAllBuiltinToolNames() {
  const { getBuiltinServers, getBuiltinTools } = loadBuiltinModule();
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          context: {
            description:
              "Optional context/background information for the Sub-Agent (e.g. 'The user is on Windows', 'Previous code analysis results').",
            type: 'string',
          },
          tools: {
            type: 'array',
            items: { type: 'string' },
            description:
              'Optional. Explicitly specify tool names to grant to the Sub-Agent. Defaults to all builtin tools if omitted.',
          },
          planning_level: {
            type: 'string',
