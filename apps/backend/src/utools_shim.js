...
function resolveFeatureCode(label) {
  ...
  if (commandMatchedCodes.length > 0) {
    const preferred = commandMatchedCodes.find((code) => String(code).endsWith(WINDOW_SHORTCUT_SUFFIX));
...
function createWindowProxy(windowId) {
  ...
  return {
    ...
    isDestroyed() { return !!query('isDestroyed'); },
    isAlwaysOnTop() { return !!query('isAlwaysOnTop'); },
    setAlwaysOnTop(flag) { action('setAlwaysOnTop', !!flag); },
    isMaximized() { return !!query('isMaximized'); },
    webContents: {
      send(channel, data) {
...
function getNativeId() {
  ...
  const created = crypto.createHash('sha256')
...
function showNotification(...args) {
  const body = args.filter(v => v !== undefined && v !== null).map(String).join(' ');
  if (!body) return;
