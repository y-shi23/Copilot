...
async function initializeMcpClient(
      ...
      const taskPromise = (async () => {
        try {
          ...
          if (saveCacheCallback && typeof saveCacheCallback === 'function') {
            ...
            saveCacheCallback(id, cleanTools).catch((e) =>
              console.error(`[MCP] Auto-cache failed for ${id}:`, e),
            );
          }
          ...
          if (error.name !== 'AbortError')
            console.error(
              `[MCP Debug] Failed to process on-demand server ${id}. Error:`,
              error.message,
            );
  ...
  if (persistentConfigsToAdd.length > 0) {
    for (const { id, config } of persistentConfigsToAdd) {
      ...
      if (config.transport === 'builtin' || config.type === 'builtin') {
        try {
          const tools = require('./mcp_builtin').getBuiltinTools(id);
      ...
      if (persistentClients.size >= 5) {
        // PERSISTENT_CONNECTION_LIMIT
        failedServerIds.push(id);
        continue;
      }
      try {
        ...
        if (saveCacheCallback && typeof saveCacheCallback === 'function') {
          ...
          const cleanTools = JSON.parse(JSON.stringify(sanitizedTools));
          saveCacheCallback(id, cleanTools).catch((e) =>
            console.error(`[MCP] Auto-cache failed for persistent ${id}:`, e),
          );
        }
...
async function invokeMcpTool(toolName, toolArgs, signal, context = null) {
  ...
  if (!toolInfo) {
    try {
      const { invokeBuiltinTool } = require('./mcp_builtin');
...
async function connectAndInvokeTool(id, config, toolName, toolArgs, context = null) {
  ...
  try {
    ...
    if (!targetTool) {
      throw new Error(
        `Tool '${toolName}' not found on server '${id}'. Available tools: ${tools.map((t) => t.name).join(', ')}`,
      );
    }
