...
const { handleFilePath, parseFileObject } = require('./file');
...
function extractMetadata(html) {
  ...
  const getMetaContent = (propName) => {
    const regex = new RegExp(
      `<meta\\s+(?:name|property)=["']${propName}["']\\s+content=["'](.*?)["']`,
      'i',
    );
...
function convertHtmlToMarkdown(html, baseUrl = '') {
  ...
  // --- 2. 移除绝对无关的标签 ---
  text = text.replace(
    /<(script|style|svg|noscript|iframe|form|button|input|select|option|textarea)[^>]*>[\s\S]*?<\/\1>/gi,
    '',
  );
  ...
  // --- 4. 基于类名/ID 的通用降噪 ---
  const noiseKeywords =
    'sidebar|comment|recommend|advert|ads|menu|login|modal|popup|cookie|auth|related|footer|copyright';
  const noiseRegex = new RegExp(
    `<div[^>]*(?:id|class)=["'][^"']*(${noiseKeywords})[^"']*["'][^>]*>[\\s\\S]*?<\\/div>`,
    'gi',
  );
  ...
  text = text.replace(/<a[^>]*href="([^"]*)"[^>]*>([\s\S]*?)<\/a>/gi, (match, href, content) => {
    const cleanContent = content.replace(/<[^>]+>/g, '').trim();
    if (!cleanContent || href.startsWith('javascript:') || href.startsWith('#'))
      return cleanContent;
  ...
  const lineNoiseRegex =
    /^(Sign in|Sign up|Log in|Register|Subscribe|Share|Follow us|Menu|Top|Home|About|Contact|Privacy|Terms)/i;
...
// --- Definitions ---
const BUILTIN_SERVERS = getBuiltinServersMetadata({ isWin });
...
const BUILTIN_TOOLS = {
  builtin_python: [
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          interpreter: {
            type: 'string',
            description: 'Optional. Path to specific python executable.',
          },
        },
        required: ['code'],
      },
    },
    {
      ...
      description:
        'Execute a local Python script file. Supports setting working directory and arguments.',
      inputSchema: {
        type: 'object',
        properties: {
          ...
          working_directory: {
            type: 'string',
            description:
              "Optional. The directory to execute the script in. If not provided, defaults to the file's directory.",
          },
          interpreter: {
            type: 'string',
            description: 'Optional. Path to specific python executable.',
  ...
  builtin_filesystem: [
    {
      name: 'glob_files',
      description:
        'Fast file pattern matching to locate file paths. Use this to find files before reading them.',
      inputSchema: {
        type: 'object',
        properties: {
          pattern: {
            type: 'string',
            description:
              "Glob pattern (e.g., 'src/**/*.ts' for recursive, '*.json' for current dir).",
          },
          path: {
            type: 'string',
            description: 'Root directory to search. Defaults to current user home.',
          },
        },
        required: ['pattern'],
      },
      ...
      inputSchema: {
        ...
        properties: {
          ...
          output_mode: {
            ...
            description:
              "Output mode: 'content' (lines), 'files_with_matches' (paths only), 'count'.",
          },
    ...
    },
    {
      name: 'read_file',
      description:
        "Read content from a local file path or a remote file. Supports text, code, and document parsing. For large files, use 'offset' and 'length' to read in chunks.",
      inputSchema: {
        type: 'object',
        properties: {
          file_path: {
            type: 'string',
            description: 'Absolute path to the local file OR a valid HTTP/HTTPS URL.',
          },
          offset: {
            type: 'integer',
            description: 'Optional. The character position to start reading from. Defaults to 0.',
            default: 0,
          },
          length: {
            type: 'integer',
            description: `Optional. Number of characters to read. Defaults to ${MAX_READ}.`,
            default: MAX_READ,
          },
        },
        required: ['file_path'],
    ...
    },
    {
      name: 'write_file',
      description:
        'Create a new file or completely overwrite an existing file. CAUTION: This tool is ONLY for TEXT-BASED files (code, txt, md, json, etc.). DO NOT use this for binary or Office files (e.g., .docx, .xlsx, .pdf, .png) as it will corrupt them.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    },
    {
      name: 'edit_file',
      description:
        "String replacement for modifying code or text files. YOU MUST READ THE FILE FIRST to ensure you have the exact 'old_string'.",
      inputSchema: {
        type: 'object',
        properties: {
          ...
          old_string: {
            type: 'string',
            description:
              'The EXACT text to be replaced. Must be unique in the file unless replace_all is true.',
          },
          new_string: { type: 'string', description: 'The new text to replace with.' },
          replace_all: {
            type: 'boolean',
            description:
              'If true, replaces all occurrences. If false, fails if old_string is not unique.',
          },
        },
    ...
    },
    {
      ...
      description:
        'Efficient replace text in a file using JavaScript RegExp. Efficient for making specific changes (e.g., renaming variables, updating arguments) without rewriting the whole file. Supports capture groups ($1, $2).',
      inputSchema: {
        type: 'object',
        properties: {
          file_path: { type: 'string', description: 'Absolute path to the file.' },
          pattern: {
            type: 'string',
            description: "The Regex pattern to search for. (e.g. 'function oldName\\((.*?)\\)')",
          },
          replacement: {
            type: 'string',
            description: 'The replacement text. Use $1, $2 for capture groups.',
          },
    ...
    },
    {
      name: 'insert_content',
      description:
        "Efficient insert content into a file. Supports two modes: 1. By 'anchor_pattern' (Recommended, safer). 2. By 'line_number' (Use ONLY if you have verified the exact line number via grep_search).",
      inputSchema: {
        type: 'object',
        properties: {
          file_path: { type: 'string', description: 'Absolute path to the file.' },
          ...
          anchor_pattern: {
            type: 'string',
            description: 'Mode A: A unique regex pattern to locate the insertion point.',
          },
          line_number: {
            type: 'integer',
            description:
              "Mode B: Absolute line number (1-based). CAUTION: Only use if you recently retrieved the line number using 'grep_search'.",
          },
          direction: {
            type: 'string',
            enum: ['before', 'after'],
      ...
      inputSchema: {
        ...
        properties: {
          ...
          timeout: {
            type: 'integer',
            description:
              'Optional. Timeout in milliseconds. Default is 15000 (15 seconds). Set higher (e.g., 300000 for 5 mins) for long-running tasks like installations.',
            default: 15000,
          },
        },
        required: ['command'],
  ...
  builtin_search: [
    {
      name: 'web_search',
      description:
        "Search the internet for a given query. Returns snippets only. Constraint: After replying, 'Sources:' citation links must be included.",
      inputSchema: {
        type: 'object',
        properties: {
          ...
          language: {
            type: 'string',
            description:
              "Preferred language/region code (e.g., 'zh-CN', 'en-US', 'jp'). Defaults to 'zh-CN'.",
          },
        },
        required: ['query'],
      },
    },
    {
      name: 'web_fetch',
      description:
        'Retrieve and parse the FULL text content of a specific URL. Use this when the user provides a URL or after getting a URL from search results. Capable of parsing complex pages like documentation, papers, and code repositories.',
      inputSchema: {
        type: 'object',
        properties: {
  ...
  builtin_subagent: [
    {
      name: 'sub_agent',
      description:
        'Delegates a complex task to a Sub-Agent. You can assign specific tools, set the planning depth, and provide context. The Sub-Agent will autonomous plan and execute.',
      inputSchema: {
        type: 'object',
        properties: {
          ...
          context: {
            type: 'string',
            description:
              'Background info, previous conversation summary, code snippets, or user constraints. Do NOT leave empty if the task depends on previous messages.',
          },
          tools: {
            type: 'array',
            items: { type: 'string' },
            description:
              'List of tool names to grant. You MUST explicitly list the tools required for the task. If omitted or empty, the Sub-Agent will have NO tools.',
          },
          planning_level: {
            type: 'string',
            enum: ['fast', 'medium', 'high', 'custom'],
            description:
              "Complexity level: 'fast'(10 steps), 'medium'(20 steps, default), 'high'(30 steps), or 'custom'.",
          },
          custom_steps: {
            type: 'integer',
            minimum: 10,
...
async function runSubAgent(args, globalContext, signal) {
  ...
  if (allowedToolNames && Array.isArray(allowedToolNames) && allowedToolNames.length > 0) {
    ...
    availableTools = (allToolDefinitions || []).filter(
      (t) => allowedSet.has(t.function.name) && t.function.name !== 'sub_agent', // 排除自身，防止递归
    );
  }
  ...
  else if (planning_level === 'custom' && custom_steps)
    MAX_STEPS = Math.min(100, Math.max(10, custom_steps));
...
- Please start by analyzing the task and available tools.`;
  ...
  log(
    `[Sub-Agent] Started. Max steps: ${MAX_STEPS}. Tools: ${availableTools.map((t) => t.function.name).join(', ') || 'None'}`,
  );

  // 动态导入
  const { invokeMcpTool } = require('./mcp');
      ...
      for (const toolCall of message.tool_calls) {
        ...
        try {
          ...
          if (typeof result === 'string') toolResult = result;
            ...
            toolResult = result.map((i) => i.text || JSON.stringify(i)).join('\n');
...
const handlers = {
  ...
  run_python_file: async (
    { file_path, working_directory, interpreter, args = [] },
    context,
    signal,
  ) => {
    return new Promise(async (resolve, reject) => {
        ...
        if (magicIndex > -1) {
          ...
          if (lastSep > -1) {
            ...
            if (!searchPath || searchPath === '~' || searchPath === os.homedir()) {
              ...
              globPattern = pattern
                .replace(normalizePath(resolvePath(searchPath)), '')
                .replace(/^[\\/]/, '');
            }
          }
        }
  ...
  // 2. Grep Search
  grep_search: async (
    { pattern, path: searchPath, glob, output_mode = 'content', multiline = false },
    context,
    signal,
  ) => {
    try {
  ...
  read_file: async ({ file_path, offset = 0, length = MAX_READ }, context, signal) => {
    try {
      ...
      if (file_path.startsWith('http://') || file_path.startsWith('https://')) {
        ...
        // 简单的 mime 推断
        const mime =
          { '.png': 'image/png', '.jpg': 'image/jpeg', '.pdf': 'application/pdf' }[ext] ||
    ...
    try {
      ...
      if (anchor_pattern) {
        ...
        try {
          ...
          return `Invalid Anchor Regex: ${e.message}`;
        }
  ...
  execute_bash_command: async ({ command, timeout = 15000 }, context, signal) => {
    return new Promise((resolve) => {
      ...
      if (dangerousPatterns.some((p) => p.test(trimmedCmd))) {
        return resolve(
          `[Security Block] The command contains potentially destructive operations and has been blocked.`,
        );
      }
      ...
      if (results.length === 0) {
        if (ddgRegion === 'cn-zh')
          return JSON.stringify({
            message: "No results found in Chinese region. Try setting language='en' or 'all'.",
            query: query,
          });
